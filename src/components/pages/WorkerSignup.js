...
const WorkerSignup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await addDoc(userCollectionRef, {email: email, state: 2002, active: true, fullTimeCount: 0, partTimeCount: 0, name: name, phone: phone, location: city, age: age, acceptedByAdmin: false, pathac: email + '-' + aadharCard.name, pathpc: email + '-' + panCard.name, pathimg: email + '-' + img.name})
      const qCR = collection(db, "rating");
      await addDoc(qCR, {workerEmail:email, ratingSum: 0, ratingCount: 0, reviews: []});
      navigate("/");
    } catch (err) {
  ...
  const uploadImage = (image) => {
    const imageRef = ref(storage,'workerIdentities/' + email + '-' + image.name);
          ...
          <Form.Group className="mb-2" controlId="formBasicCity">
            <div>
              ...
              {imgUploaded ? (
                ...
                <p>Upload Your Photo in .JPG:  <Button variant="secondary" onClick={() => handleClick("img")}>
                Upload
              </Button></p>
              )}
              {!imgUploaded && (
