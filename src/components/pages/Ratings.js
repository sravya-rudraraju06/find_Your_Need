...
function Ratings() {
  ...
  const handleSubmit = async (e) => {
    ...
    await updateDoc(matchedDoc, {ratingSum: tot + number, ratingCount: cnt + 1})
        ...
        console.log(review)
        await updateDoc(matchedDoc, {
            reviews: arrayUnion(review)
        });
      const ratingsCollectionRef = collection(db, "ratingsHistory");
      await addDoc(ratingsCollectionRef, {user: user?.email, worker: workerEmail, rating: number, review: review})
    ...
    <div className="Rating">
      ...
      <Form onSubmit={handleSubmit}>
        <div className="content">
          ...
          <textarea placeholder={handlePlaceHolder()} onChange={(e) => setReview( e.target.value)}></textarea>&nbsp;
